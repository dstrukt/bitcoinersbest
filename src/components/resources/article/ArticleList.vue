...
<div id="ArticleList">
  <div class="article-module d-flex br-4 mb-30 fade-in" v-for="article in articles" :key="article.resItem.id">
    <p class="c-white">{{article.resItem.id}}</p>
    <div class="flex-grow-1 align-self-center">
        <img :src="article.resItem.image" class="card-img" alt="...">
        <h6 class="text-uppercase c-gray-1 medium mt-25 mb-15">{{article.resItem.created_by}}</h6>
        <router-link :to="'/'+article.resItem.id+'/'+article.resItem.title"><h5 class="medium mb-15 c-white">{{article.resItem.title}}</h5></router-link>
        <h5 v-if="article.resItem.description.length < 140" class="regular c-gray-1">{{article.resItem.description}}</h5>
        <h5 v-else class="regular c-gray-1">{{ article.resItem.description.substring(0,140) + "..." }}</h5>
    </div>
    <div class="align-self-top pl-20">
        <div class="votes text-center">
          <p class="c-white">{{article.statusType.id}}</p>
          <b-button v-if="article.statusType.id != 70" class="vote btn btn-transparent bg-brand icon-wrap" @click="upVote(article.id, 1)">
          ...
          <b-button v-else="article.statusType.id == 70" class="vote btn btn-transparent bg-brand icon-wrap" v-b-modal.invoice-modal>
            ...
            <div class="vote-total" id="vote-total">
              <h6 class="medium mt-10 c-white">{{article.resItem.vote_count}}</h6>
...
export default {
  ...
  // },
    ...
    loadArticles(page = this.pageNo) {
			this.isLoading = true;
			const _self = this;
        axios.get("http://bitcoinersbest.local:9111/v1/items?access-token=admin-bandit-authkey&V1ResItemSearch[res_type_id]=30&page=" + page)
        .then(response => {
            let tempArticles = response.data;

            let articlesWithVotes = tempArticles.map(a => {
                console.log(a.id);
               let articleVotes = null;

                axios.get("http://bitcoinersbest.local:9111/v1/items/"+a.id+"/votes/69?access-token=admin-bandit-authkey")
                  .then(response2 => {
                      // articleVotes = response2.data;
                      // console.log(articleVotes);
                      for (let i in response2.data) {
                        _self.articles.push(response2.data[i]);
                      }
